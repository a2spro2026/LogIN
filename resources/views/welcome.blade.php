...
<div x-data="{
        ...
        openAdmin() { this.adminOpen = true; this.partnerModal = null; this.menu = null; this.mobileOpen = false; },
        closeAdmin() { this.adminOpen = false; },
        openPartnerLogin() { this.partnerModal = 'login'; this.adminOpen = false; this.menu = null; this.mobileOpen = false; },
        openPartnerRegister() { this.partnerModal = 'register'; this.adminOpen = false; this.menu = null; this.mobileOpen = false; },
        closePartner() { this.partnerModal = null; },
        anyModalOpen() { return this.adminOpen || !!this.partnerModal; }
     }"
     @keydown.escape.window="menu = null; mobileOpen = false; closeAdmin(); closePartner()"
...
<header class="absolute inset-x-0 top-0 z-50">
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        ...
        <div class="flex items-center gap-2">
            <button type="button" class="rounded-lg p-2 text-white transition hover:bg-white/10 lg:hidden" @click="mobileOpen = !mobileOpen" aria-label="Menu">
    ...
    <div x-show="mobileOpen" x-cloak x-transition class="border-t border-white/10 bg-navy-950/95 px-5 py-4 backdrop-blur lg:hidden">
        <div class="space-y-1 text-sm text-white/90">
            <a href="#accueil" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-gold-400" @click="mobileOpen=false">
                <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25"/></svg>
                Accueil
            </a>

            <p class="flex items-center gap-2 px-3 pt-3 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-gold-400">
                <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"/></svg>
                Nos Agences
            </p>
            @foreach ($villes as $ville)
                <a href="{{ url('/?ville='.urlencode($ville)) }}#biens" class="flex items-center gap-3 rounded-lg px-3 py-1.5 text-white/75" @click="mobileOpen=false">
            ...
            @foreach ($typesBiens as $type => $icone)
                <a href="{{ url('/?type='.urlencode($type).'&cat=louer') }}#biens" class="flex items-center gap-3 rounded-lg px-3 py-1.5 text-white/75" @click="mobileOpen=false">
                    <span class="h-1.5 w-1.5 rounded-full bg-gold-500/70"></span>
                    {{ $type }}
                </a>
            @endforeach
            ...
            @foreach ($typesBiens as $type => $icone)
                <a href="{{ url('/?type='.urlencode($type).'&cat=neuf') }}#biens" class="flex items-center gap-3 rounded-lg px-3 py-1.5 text-white/75" @click="mobileOpen=false">
                    <span class="h-1.5 w-1.5 rounded-full bg-gold-500/70"></span>
                    {{ $type }}
                </a>
            @endforeach

            <a href="#services" class="mt-2 flex items-center gap-3 rounded-lg px-3 py-2.5" @click="mobileOpen=false">
                <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 0 0 2.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456ZM16.894 20.785 16.5 21.75l-.394-.965a1.5 1.5 0 0 0-1.079-1.078l-.965-.394.965-.394a1.5 1.5 0 0 0 1.079-1.078l.394-.965.394.965a1.5 1.5 0 0 0 1.078 1.078l.965.394-.965.394a1.5 1.5 0 0 0-1.078 1.078Z"/></svg>
                Nos Services
            </a>
            <button type="button" class="mt-2 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left" @click="mobileOpen=false; openPartnerLogin()">
                <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"/></svg>
                Partenaire
            </button>
            <button type="button" class="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left" @click="mobileOpen=false; openAdmin()">
                <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z"/></svg>
                Admin
            </button>
        </div>
...
{{-- Hero --}}
<section id="accueil" class="relative min-h-[100svh] overflow-hidden">
    <img src="https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=2200&q=80"
         alt="Intérieur d'appartement de standing"
    ...
    <div class="relative z-10 mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-center px-5 pb-36 pt-28 lg:px-8 lg:pb-40">
        ...
        <h1 class="login-anim-up login-anim-up-delay-1 mt-5 max-w-2xl font-sans text-3xl font-semibold leading-[1.15] tracking-tight text-white sm:text-4xl lg:text-5xl">
            La meilleure façon de louer votre futur
            <em class="font-serif font-medium italic text-gold-400">chez-vous</em>
        </h1>

        <p class="login-anim-up login-anim-up-delay-2 mt-5 max-w-lg text-base leading-relaxed text-white/80 sm:text-lg">
    ...
    <div class="login-search-enter absolute inset-x-0 bottom-0 z-20 translate-y-1/2 px-4 sm:px-6 lg:px-8">
        <form action="#biens" method="get"
              class="mx-auto grid max-w-7xl gap-3 rounded-2xl bg-white p-3 shadow-2xl shadow-navy-950/15 sm:grid-cols-2 lg:grid-cols-[1.1fr_1.2fr_1fr_1fr_auto] lg:items-end lg:gap-2 lg:p-4">
...
<section id="services" class="bg-navy-50/80 pb-16 pt-28 sm:pt-32">
